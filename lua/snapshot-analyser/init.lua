...
function M.analyse()
    ...
    draw_data(header, entries)
    vim.api.nvim_create_user_command("SnapHide", hide_entry, { nargs = '?' })
    vim.api.nvim_create_user_command("SnapShow", show_entry, { nargs = '?' })
    vim.api.nvim_create_user_command("SnapHideCol", hide_column, { nargs = '?' })
    vim.api.nvim_create_user_command("SnapShowCol", show_column, { nargs = '?' })
    vim.api.nvim_create_user_command("SnapColor", add_highlight, { nargs = '?' })
    vim.api.nvim_create_user_command("SnapClear", remove_highlight, { nargs = '?' })
    vim.api.nvim_create_user_command("SnapAdd", jumplist_add, { nargs = '?' })
    vim.api.nvim_create_user_command("SnapRemove", jumplist_remove, { nargs = '?' })
end
